...
class AuthTransformer extends graphql_transformer_core_1.TransformerAuthBase {
    constructor() {
        ...
        this.addIamAuthToCustomTypes = (ctx) => {
            if (!ctx.transformParameters.sandboxModeEnabled && !ctx.synthParameters.enableIamAccess) {
                return;
            }
            const needsAwsIamDirective = (type) => {
                var _a;
                return !((_a = type.directives) === null || _a === void 0 ? void 0 : _a.some((dir) => dir.name.value === 'aws_iam'));
            };
            const isNonModelType = (type) => {
                var _a;
                return !((_a = type.directives) === null || _a === void 0 ? void 0 : _a.some((dir) => dir.name.value === 'model'));
            };
            const isNotInterface = (type) => {
                return type.kind !== graphql_1.Kind.INTERFACE_TYPE_DEFINITION;
            };
            ctx.inputDocument.definitions
                .filter(graphql_transformer_core_1.isObjectTypeDefinitionNode)
                .filter(isNonModelType)
                .filter(needsAwsIamDirective)
                .filter(isNotInterface)
                .forEach((def) => (0, utils_1.extendTypeWithDirectives)(ctx, def.name.value, [(0, graphql_transformer_common_1.makeDirective)('aws_iam', [])]));
        };
