...
const matchDirective = (definition, directive, node) => {
    ...
    if (node.kind === graphql_1.Kind.OBJECT_TYPE_EXTENSION || node.kind === graphql_1.Kind.INTERFACE_TYPE_EXTENSION) {
        throw new errors_1.InvalidDirectiveError(`Directives are not supported on object or interface extensions. See the '@${directive.name.value}' directive on '${node.name.value}'`);
    }
