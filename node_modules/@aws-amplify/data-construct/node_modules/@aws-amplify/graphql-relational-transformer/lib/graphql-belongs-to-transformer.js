...
const validate = (config, ctx) => {
    ...
    if (config.overrideIndexName && !config.references) {
        throw new graphql_transformer_core_1.InvalidDirectiveError(`overrideIndexName cannot be used on @${graphql_directives_1.BelongsToDirective.name} without references. Modify ${object.name.value}.${field.name.value} to use references or remove overrideIndexName.`);
    }
    let dbType;
    try {
